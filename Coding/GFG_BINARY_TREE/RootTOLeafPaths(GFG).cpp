...
class Solution {
  public:
   void root_to_leaf_path(Node* node,vector<int>&temp,vector<vector<int>>& ans)
     {
        if(!node) return;
    ...
    vector<vector<int>> Paths(Node* root) {
        ...
        vector<int>temp;
        root_to_leaf_path(root,temp,ans);
        return ans;
    }
};

// SIMILAR PROBLEM --->  Root to given Node Path in Binary Tree
